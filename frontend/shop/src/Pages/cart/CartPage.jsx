...
const CartPage = () => {
    ...
    const handleSelectDelete = async () => {
        ...
        try {
            await Promise.all(
                checkedItems.map(cartId => axios.delete(`/api/cart/delete/${cartId}`))
            );
            fetchCartItems();
            setCheckedItems([]);
    ...
    const handleClearCart = async () => {
        try {
            await axios.delete(`/api/cart/clear/${memberId}`);
            fetchCartItems();
            setCheckedItems([]);
            setIsAllChecked(false);
            alert("장바구니 비움 완료");
    ...
    const handleQuantityChange = async (cartId, newQty) => {
        try {
            await axios.put(`/api/cart/update/${cartId}`, { qty: parseInt(newQty, 10) || 1 });
            fetchCartItems();
        } catch (error) {
